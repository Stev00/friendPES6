...
set "HOSTIP=%~1"
if "%HOSTIP%"=="" set /p "HOSTIP=请输入 PChost 公网 IP (输入后按回车): "
if "%HOSTIP%"=="" (
  echo   [X] 未输入 IP, 中止
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$f=\"$env:WINDIR\System32\drivers\etc\hosts\"; $b=$f+'.pes6bak'; if(!(Test-Path $b)){Copy-Item $f $b}; $c=Get-Content $f | Where-Object {$_ -notmatch 'winning-eleven\.net'}; $c+=('%HOSTIP%'+' pes6gate-ec.winning-eleven.net'); $c+=('%HOSTIP%'+' we9stun.winning-eleven.net'); [IO.File]::WriteAllLines($f,$c)"
