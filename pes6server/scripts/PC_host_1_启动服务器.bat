...
if errorlevel 1 (
  start "PES6-STUN" /min "%ROOT%\stun\dist\pes6-stun.exe" --public-ip %PUBIP% --lan-ip %LANIP% --relay-port 5731 --game-port 5730
echo 【信息】 STUN 已启动
) else (
echo 【信息】 STUN 已在运行, 跳过
)
ping -n 3 127.0.0.1 >nul
netstat -ano -p udp | findstr ":5731" >nul && echo 【OK】 STUN 中继 5731 已监听(对战流互转+RTT 测量) || echo 【X】 中继 5731 未监听, 对战会黑屏
