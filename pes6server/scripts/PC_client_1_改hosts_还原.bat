...
echo [版本] PC_client_1_改hosts_还原 v1.0 (2026-09-05)
set "SELF=%~f0"
if not exist "%SELF%" (
  pushd "%~dp0" 2>nul
  set "SELF=%CD%\%~nx0"
...
if "%RESTORE_FROM%"=="bak" (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$f=\"$env:WINDIR\System32\drivers\etc\hosts\"; $c=Get-Content $f | Where-Object {$_ -notmatch 'winning-eleven\.net'}; [IO.File]::WriteAllLines($f,$c)"
...
echo   [OK] DNS 缓存已刷新
echo.
echo [完成] hosts 还原结束, 本机不再连接 PES6 服务器
if not defined NOPAUSE pause
